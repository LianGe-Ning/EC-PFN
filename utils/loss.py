...
# class ComputeLoss:     # '''new'''
#     # Compute losses
#     def __init__(self, model,  use_qfl=False, use_dfl=True):
...
#                 # iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False, FocalLoss_='EIoU',EIoU=True)  # iou(prediction, target)
#                 # iou_ori = iou[0]
#                 # iou_f = iou[1]
#                 # if len(iou) >= 2:
...
#
#     def forward(self, pred, true):
#         loss = self.loss_fcn(pred, true)  # 使用平衡的 Focal Loss计算预测结果的损失值
#         loss = loss.flatten(1)  # 将三维单元格结构展平
#
#         # 创建权重向量矩阵，与损失值矩阵保持相同的形状，并将每个图像样本的损失值复制到对应的权重向量中
#         weights = torch.zeros_like(loss)
#         for i, l in enumerate(loss):
#             weights[i] = l
#
#         weights = weights.topk(int(weights.size()[1] * (1 - self.LRM_ignore)), dim=1)[0]  # 对每个图像样本的权重进行排序，选择前B个单元格数量比例
#         if self.reduction == 'mean':
#             return weights.mean()  # 分别计算每个选定权重的均值
#         elif self.reduction == 'sum':
#             return weights.sum()  # 分别计算每个选定权重的总和
#         else:  # 'none'
#             return weights  # 返回排序后的权重向量
...
class ComputeLoss:               # '''new——5.6    Loss Rank Mining'''
    sort_obj_iou = False
...
class ComputeLossOTA:          #'''new_wious '''
    # Compute losses
    def __init__(self, model, autobalance=False):
        super(ComputeLossOTA, self).__init__()
        device = next(model.parameters()).device  # get model device
    ...
    def __call__(self, p, targets, imgs, category_iou=None):  # predictions, targets, model
        device = targets.device
        lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
        bs, as_, gjs, gis, targets, anchors = self.build_targets(p, targets, imgs)
        pre_gen_gains = [torch.tensor(pp.shape, device=device)[[3, 2, 3, 2]] for pp in p]
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                if category_iou == 'wise_iou':
                    loss, iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, WIoU=True)  # iou(prediction, target)
