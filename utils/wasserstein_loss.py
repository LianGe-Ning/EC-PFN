...
from utils.general import box_iou, bbox_iou
...
class FocalLoss(nn.Module):
    ...
    def forward(self, pred, true):
        ...
        # https://zhuanlan.zhihu.com/p/596437675
...
def box_iou_v(box1, box2, x1y1x2y2=True):
...
def repulsion_loss_t(pbox, gtbox, deta=0.5, pnms=0.1, gtnms=0.1, x1x2y1y2=False):
    repgt_loss = 0.0
    repbox_loss = 0.0
    pbox = pbox.detach()
    gtbox = gtbox.detach()
    ...
    pgiou = box_iou_v(pbox, gtbox, x1y1x2y2=x1x2y1y2)
    pgiou = pgiou.cuda().data.cpu().numpy()
    ppiou = box_iou_v(pbox, pbox, x1y1x2y2=x1x2y1y2)
...
class WDComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        print('use Normalized Gaussian Wasserstein Distance Loss🚀')
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                for id, indexs in dic.items():  # id = batch_name  indexs = target_id
                    if indexs:
                        lrepgt, lrepbox = repulsion_loss_t(pbox[indexs], tbox[i][indexs], deta=deta, pnms=Rp_nms, gtnms=Rp_nms)
                        _lrepGT += lrepgt
                        _lrepBox += lrepbox
                        bts += 1
                if bts > 0:
