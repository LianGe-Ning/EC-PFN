...
class Encoding(nn.Module):
    def __init__(self, in_channels, num_codes):
        ...
        self.codewords = nn.Parameter(
            ...
            # 并将这个parameter绑定到这个module里面。即在定义网络时这个tensor就是一个可以训练的参数了。使用这个函数的目的也是想让某些变量在学习的过程中不断的修改其值以达到最优化。
        # [num_codes]
    ...
    def aggregate(assignment_weights, x, codewords):
        ...
        # ---处理特征向量x b, c1, N     unsqueeze 作用是怎加一个维度
        expanded_x = x.unsqueeze(2).expand((b, x.size(1), num_codes, in_channels))
...
class GroupNorm(nn.GroupNorm):     #分组做归一化
    """
...
class EVCBlock(nn.Module):
    def __init__(self, in_channels, out_channels, channel_ratio=4, base_channel=16):
        ...
        # LVC
        self.lvc = LVCBlock(in_channels=in_channels, out_channels=in_channels, num_codes=64)  # c1值暂时未定时
        # LightMLPBlock
        self.l_MLP = LightMLPBlock(in_channels, in_channels, ksize=1, stride=1, act="silu", act_layer=nn.GELU, mlp_ratio=4., drop=0.,
