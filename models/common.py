...
#########_____new_add_____________________
...
class ConvNextBlock_1(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6):
        ...
        self.dwconv = nn.Conv2d(dim, dim, kernel_size=7, padding=3, groups=dim)  # depthwise conv
        self.norm = LayerNorm_s(dim, eps=1e-6)
        self.nconv1 = nn.Conv2d(dim, 4 * dim, 1, 1)
        self.act = nn.GELU()
        self.nconv2 = nn.Conv2d(4 * dim, dim, 1, 1)
        self.gamma = nn.Parameter(layer_scale_init_value * torch.ones((dim)),
                                  requires_grad=True) if layer_scale_init_value > 0 else None
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
...
class sSE_Conv(nn.Module):
    # Standard convolution
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super(sSE_Conv, self).__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        self.cse1 = sSE(c2)
...
class CsS_Conv(nn.Module):  # SCSE 的通道和CBAM空间注意力结合
    # Standard convolution
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
    ...
    def forward(self, x):
        #         x1=self.conv1(x)
        y = self.Cse(self.act(self.bn(self.conv(x))))
        return self.Space(y) + y

    def fuseforward(self, x):
        return self.act(self.conv(x))

class CsS_Conv_1(nn.Module):  # SCSE 的通道和CBAM空间注意力结合____尝试修改
    # Standard convolution
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super(CsS_Conv_1, self).__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        self.Cse = cSE(c2)
...
class GSConv(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):
        ...
        c_ = c2 // 2
        self.cv1 = Conv(c1, c_, k, s, None, g, act)
        self.cv2 = Conv(c_, c_, 5, 1, None, g, act)   #self.cv2 = Conv(c_, c_, 5, 1, None, c_, act)
        self.cv3=Conv(2*c_, c2, 1, 1, None, g, act)
    def forward(self, x):
        x1 = self.cv1(x)
...
class C3GhostV2(nn.Module):
    # C3GV2 module with GhostV2(for iscyy)
    def __init__(self, c1, c2, n=5, shortcut=True, g=1, e=0.5):
        super().__init__()
        self.c1_ = 16
        #self.c2_ = 16 * e
        c_ = int(c2 * e)
        self.conv_C3 = C3(c1, c_, 1)
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(2 * c_, c2, 1)
        self.m = nn.Sequential(*(Ghostblockv2(c_, self.c1_, c_) for _ in range(n)))

    def forward(self, x):
        return self.cv2(torch.cat((self.m(self.conv_C3(x)), self.cv1(x)), 1))
...
#-----------------------------------
class HorBc(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super(HorBc, self).__init__()
        c_ = int(c2 * e)
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(c_, c_, 1, 1)
...
class ContrastEnhance(nn.Module):
    def __init__(self, contrast=2.0):
        super(ContrastEnhance, self).__init__()
        self.color_jitter = transforms.ColorJitter(contrast=contrast)

    def forward(self, x):
        x = x.cpu()
        for i in range(x.size(0)):
            x[i] = self.color_jitter(x[i])
        x = x.cuda()
        return x
#-----------------------------------
class SCE(nn.Module):
    ...
    def forward(self, x):
        x_p1, x_p2 = x
        x = torch.cat([self.down(x_p1), x_p2], dim=1)
        return x
