...
class RepConv_L(nn.Module):

    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, dilation=1, groups=1,
                 padding_mode='zeros', deploy=False, act='relu', norm=None):
        super(RepConv_L, self).__init__()
        self.deploy = deploy
        self.groups = groups
...
class BasicBlock_3x3_Reverse(nn.Module):
    def __init__(self, ch_in, ch_hidden_ratio, ch_out, act='relu', shortcut=True):
        ...
        self.conv2 = RepConv_L(ch_in, ch_hidden, 3, stride=1, act=act)
...
class UniFusionLayer(nn.Module):
    # CSPStage BLock
    def __init__(self, ch_in=768, ch_out=512, ch_hidden_ratio=1.0, n=round(5 * 1.0), block_fn='BasicBlock_3x3_Reverse',
                 act='swish', spp=False):
        super(UniFusionLayer, self).__init__()
        split_ratio = 2
...
def conv_bn(in_channels, out_channels, kernel_size, stride, padding, groups=1):
    '''Basic cell for rep-style block, including conv and bn mg'''
