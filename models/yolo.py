...
from models.asff import ASFFDetect
from models.cspnext import CSPNeXtLayer
from models.repfpn import RepVGGBlock, SimSPPF, SimConv, RepBlock, Transpose
from models.convnextv2 import ConXBv2
from models.qarepvgg import QARepNeXt
#from models.efficientrep import RepVGGBlock, SimSPPF, SimConv, RepBlock, Transpose
from models.UniFusionLayer import UniFusionLayer, RepConv_L
from models.odconv import ODConv
from models.dodconv import DODConv
from models.CBAM_4_3 import CBAM_4_3
...
class Model(nn.Module):
    def __init__(self, cfg='yolor-csp-c.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, TDetect):
            ...
            forward = lambda x: self.forward(x)[0] if isinstance(m, (Segment, TDetect)) else self.forward(x)
        ...
        if isinstance(m, (Decoupled_Detect, TSCODE_Detect)):
            s = 256  # 2x min stride
            #m.inplace = self.inplace
    ...
    def forward_once(self, x, profile=False):
        ...
        for m in self.model:
            ...
            if self.traced:
                ...
                if isinstance(m, Detect) or isinstance(m, IDetect) or isinstance(m, IAuxDetect) or isinstance(m, IKeypoint) or isinstance(m, ASFFDetect) or isinstance(m, Decoupled_Detect) or isinstance(m, TSCODE_Detect):
                    break

            if profile:
                #c = isinstance(m, (Detect, IDetect, IAuxDetect, IBin))
                c = isinstance(m, (Detect, IDetect, IAuxDetect, IBin, ASFFDetect, Decoupled_Detect,TSCODE_Detect))
    ...
    def fuse(self):  # fuse model Conv2d() + BatchNorm2d() layers
        ...
        for m in self.model.modules():
            if isinstance(m, RepConv):
                ...
                m.fuse_repvgg_block()
            elif isinstance(m, RepConv_L):
                print(f"RepConv_L.switch_to_deploy")
                m.switch_to_deploy()
            ...
            elif isinstance(m, (IDetect, ASFFDetect, Decoupled_Detect,TSCODE_Detect, IAuxDetect)):
                m.fuse()
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [nn.Conv2d, Conv, CAConv, BasicRFB, ECA, Conv_CBAM, CsS_Conv, CsS_Conv_1, GSConv, BoT3, CoT3, C3, C3HB, RobustConv, RobustConv2, DWConv, GhostConv, RepConv, RepConv_OREPA, DownC,
            ...
            args = [c1, c2, *args[1:]]
            if m in [DownC, BoT3, CoT3, C3, C3HB, SPPCSPC, GhostSPPCSPC,
            ...
            if m in [QARepNeXt]:
                args.insert(2, n)  # number of repeats
                n = 1
        elif m in [UniFusionLayer]:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not outputss
                c2 = make_divisible(c2 * gw, 8)
            args = [c1, c2, *args[1:]]
            if m in [UniFusionLayer]:
                args.insert(2, n)  # number of repeats
                n = 1
        elif m in [ODConv]:
            c1, c2 = ch[f], args[0]
